...
int main()
{
	...
	const unsigned int substeps = 1;//8;
	...
	window.Show([&](double dt)
	{
		...
		{
			float r = std::clamp(rand() / (float)RAND_MAX * 10.0f, 3.5f, 10.0f) * 3.0;
			float m = 1.0f;
			...
			ecs.CreateEntity(Transform(Matrix4::PositionScale2d(pos, r)), RenderColor(col), PhysicsCircle(r, m, pos, acc));
			physicsObjCount++;
